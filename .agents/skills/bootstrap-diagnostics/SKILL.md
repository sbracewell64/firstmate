...
- `CAPACITY_DEFERRED: <n> task(s) stopped waiting for model capacity and were never dispatched - <ids>` - each named task was held because every model meeting its capability floor was out of provider capacity, waited through its bound, and stopped.
  ...
  Read the recorded reason and the current picture with `bin/fm-route.sh capacity --route <id>`, then decide one of three things and act on it rather than re-dispatching by hand: the wait is still worth making and the bound is raised deliberately with `bin/fm-attempt.sh defer <id> --defer-budget <n>`; the work is worth doing at a different capability floor, which is a routing decision made explicitly and never a substitution made because a model happened to be free; or the work is no longer wanted, and `bin/fm-capacity-retry.sh release <id>` drops the wait.
