...
- `OUTBOUND: <item> is waiting on <gate> with no applicable durable artifact (<token>) - <evidence>` - the item's durable state says it is waiting for something outside the fleet, and the thing it is waiting for was never created or no longer applies to its current head.
  ...
  Read the token to choose the repair: `FM_OUTBOUND_NO_ARTIFACT` means nothing was ever asked; `FM_OUTBOUND_STALE_HEAD` means an earlier ask exists but the reviewed head moved, so it answers a different question now; `FM_OUTBOUND_INCOMPLETE_BINDING` means a structural field the identity needs - usually the gate type - is absent, so the item cannot be bound to an exact head at all and must be typed before anything is asked rather than papered over with a vague request.
  `FM_OUTBOUND_HEAD_UNOBSERVED` is NOT on this list: an unreadable head is a failed observation and appears under could-not-observe below.
...
- `OUTBOUND: <item> - its exact head could not be read from the declaration, the forge or the clone (FM_OUTBOUND_HEAD_UNOBSERVED), so this sweep did not look for an artifact and is NOT reporting that none exists` - the item's typed gate is fine and only its exact head could not be observed, so the sweep never reached the forge for it.
  This is could-not-observe and never a defect: an artifact bound to that head may well exist, and the item may equally be genuinely stranded, so do not tell the captain either way from this line.
  Repair the READ - restore the clone, the declaration's `head`, or the pull-request reference the item names - and let the next sweep classify it; emitting a request here would ask about a head nobody has established.
  Most of the remaining tokens name which read FAILED, and each of those is repaired by restoring that read rather than by re-emitting anything; the one exception is `FM_OUTBOUND_AMBIGUOUS_CANDIDATES` below, where the read succeeded and the answer itself is unusable.
  ...
  `FM_OUTBOUND_AMBIGUOUS_CANDIDATES` - the read SUCCEEDED and returned more than one candidate, so nothing is broken and nothing can be chosen either; the line names the count.
  Do not debug the probe, and do not read this as an absent artifact: on the `pull-request` channel it means the exact head carries more than one open pull request, and the repair is to close or retarget the extra one so a single request answers for that head.
  ...
  `FM_OUTBOUND_SUBJECT_UNRESOLVED` - the item's clone names more than one repository and nothing declares which one the review is ABOUT, so the request identity cannot be computed and no artifact can be looked for; declare the governed repository in that item's `outbound-gate.json`, and note that this is not the same as the venue being unresolved - the venue is where the question gets asked, never what it is about.
  ...
  `FM_OUTBOUND_DONE_ARCHIVE_UNREADABLE` - the completion archive EXISTS and could not be read, so the lifecycle corpus was never finished and no verdict may be drawn from the backlog alone.
  This one is not a gap in the records but a permissions or I/O fault on a file that is present: repair the archive's readability rather than looking for a completion row that was there all along.
