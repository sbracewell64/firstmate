...
- `VALIDATION_DAEMON: down - <evidence>` - the shared no-mistakes validation daemon is not running, so no task can be validated and every pipeline already waiting on it is stalled.
  The line carries how long it has been down; a multi-day figure means work has been silently unvalidated for that long, which is the exact failure this check exists to end.
  ...
  Report the stall and its length to the captain under `AGENTS.md` section 9 before dispatching work that depends on validation.
