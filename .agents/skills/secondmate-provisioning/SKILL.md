...
The locked session-start deferred network stage runs the same bootstrap sweep for every live local secondmate home, discovered from `state/<id>.meta` records with `role=secondmate` (`data/secondmates.md` only backfills `home=` for older records).
