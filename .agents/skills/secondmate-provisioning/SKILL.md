...
`config/qualification-contract-dir` travels on the same terms and for the same reason: the inherited dispatch rules are checked against capability contracts, so a home answering the same route's requirement out of a different contract generation is exactly the divergence that binding closes.
An absolute binding keeps the whole fleet on one directory and a home-relative one converges each home on its own copy; `docs/configuration.md` "Role qualification register" owns its schema and refusal semantics.
