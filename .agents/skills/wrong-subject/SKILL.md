...
The named-path input and the stdin input each carried that gap, and each had to be closed on its own.
...
Two surfaces of one small tool is the part worth keeping: a single slip would say only that someone was careless, while a recurrence says the class is structural and rides on whatever a check happens to touch.
...
None of that was evidence against the class, which is why a reviewer needs to be able to name it rather than argue it into existence each time.
