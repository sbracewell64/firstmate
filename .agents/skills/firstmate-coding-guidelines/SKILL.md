...
- A suite must be runnable from any worktree on any branch, so no case may read the git state of the checkout it executes in: give a script that needs a firstmate primary checkout a scratch one and point `FM_ROOT_OVERRIDE` at it, because `fail` exits and one case that reads the executing checkout takes every later case in the file down with it.
- A guard a case must not trip is still behavior, so measure it: assert its absence where the case needs it silent, and give it its own case that constructs the tripping state and asserts the refusal, rather than leaving it exercised only when the environment happens to trip it.
