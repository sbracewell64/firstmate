...
- Two bash behaviors produce exactly that empty path: `local a=$1 b="$a/x"` leaves `b` unset under `set -u` because bash expands every `local` right-hand side before assigning any of them, and `fail` inside a command substitution kills only the subshell, so a fixture that echoes its path hands the caller an empty string and keeps going.
