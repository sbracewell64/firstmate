...
- After the pipeline pushes the branch, publish the head-bound attestation with `bin/fm-attest.sh write --only-if-required`, because the `Require no-mistakes` check reads that and not the pull request body, and every new head needs its own ([`docs/no-mistakes-attestation.md`](../../../docs/no-mistakes-attestation.md) owns who publishes, when, and the condition that closes the recurrence of unpublished evidence).
...
- A test that is itself a subject of a generated acceptance artifact must never require that artifact to be CURRENT for its own bytes, because the edit that needs re-measuring is the edit that makes the test fail ([`docs/architecture.md`](../../../docs/architecture.md), EVIDENCE_GENERATION_WELL_FOUNDEDNESS).
