...
`pi` and `pi-signed` also block directly, because a follow-up queued from Pi's `agent_end` callback continues the same run and suppresses the idle signal.
`opencode` exposes passive lifecycle callbacks and forces one bounded follow-up when the shared predicate blocks.
