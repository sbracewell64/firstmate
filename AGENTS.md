...
state/               volatile runtime signals; gitignored
  ...
  model-observation.json  narrow mode-0600 record of what each model's LAST PROBE OBSERVED, three-valued and written only by bin/fm-model-verify.sh; bin/fm-availability-lib.sh owns the type and the record
                          A probe that could not run is UNOBSERVABLE and carries a TOOLING_GAP evidence block naming the reader to repair: it excludes the candidate from routing but is never a hold, because a broken reader is not a provider fact and releasing a hold would not fix it
