...
Run `bin/fm-pr-check.sh <id> <PR url>` - it resolves which request actually lands for this task, records `pr=` and the forge's `pr_head=` when available in the task's meta, and arms the watcher's merge poll, refusing rather than watching a request that will not land here.
