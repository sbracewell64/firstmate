...
An unregistered project resolves to `no-mistakes` with yolo off and the registration gap goes to the captain; a home with no registry keeps the task's recorded yolo posture because no canonical entry exists there to supersede it.
...
- `sssf-planning-awareness` - load on any check wake naming `sssf-planning`, before acknowledging a planning event, and before treating a planning state as a reason to start work.
  A planning transition is awareness; only `ACTIVE` reaches ordinary engineering intake, and even `ACTIVE` is eligibility rather than authority, so the named increment is still fetched at its exact source identity and admitted normally before any work exists.
