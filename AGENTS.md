...
When a trunk repair is queued and the pool has no free slot, `bin/fm-slot-reservation.sh` holds the NEXT slot that frees for that one task, and the guard then refuses every other dispatch that slot; it creates no capacity, never preempts a running lane, and expires, so read that script's header before opening, releasing, or explaining one.
