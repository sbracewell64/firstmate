...
The spawn itself confirms the brief reached the worker and reports `FM_SPAWN_DELIVERY_UNCONFIRMED` instead of success when it could not observe that, which is could-not-observe and never a failed spawn: inspect and steer that lane rather than re-dispatching it, handle any trust dialog through `harness-adapters`, and record ship or scout work as under way.
