...
That refusal is the control: a reviewer must be a binding the role qualifies, the author identity is required rather than assumed, an absent one refuses because it cannot be told from a self-review, and the harness must be one with a measured read-only launch binding, which is then substituted into the launch command so the measured mutation surface is removed rather than relying on an instruction not to write.
Passing that check is an intention and never evidence that a review happened: before concluding one did, ask `bin/fm-review-role.sh assignment --task <id>`, which requires the recorded launch outcome plus every other fact its schema names and otherwise leaves the obligation unsatisfied.
Never conclude a review from a spawn having been attempted, from a quiet endpoint, or from the reviewer having written nothing, because a reviewer that never started looks exactly like one that ran and lawfully declined to write.
