...
A candidate becomes ATTRIBUTABLE one step earlier still, when it is committed, so every generated ship brief binds the production commit identity through `bin/fm-commit-identity.sh` before the worker's first commit: it installs the publication identity policy's declared author and committer into both the task checkout and the repository the validation pipeline commits in, refuses when that identity is missing, placeholder, ambiguous or unobservable, and a worker reporting one of its refusals is blocked on identity configuration rather than on the task.
