...
state/               volatile runtime signals; gitignored
  ...
  <id>.attempt       durable attempt count and retry budget for the task id (attempt=, attempt_budget=, failures=, terminal=), plus capacity-deferral observations used for retry backoff (deferrals=, defer_signature=, defer_stagnant=), owned by bin/fm-attempt.sh; spent only by a ship or scout spawn that follows a RECORDED FAILURE, while a spawn after a dead runtime or husk continues the attempt already open, and retired by an ordinary teardown but kept under --force, so a retry decision is arithmetic rather than a judgment
  <id>.capacity      durable record of a dispatch DEFERRED because no model meeting its floor had capacity, written only by bin/fm-capacity-retry.sh, which also resumes it automatically once the recorded retry condition is met; observations are counted by bin/fm-attempt.sh as deferrals= without spending an attempt or becoming a stop authority (docs/configuration.md "Capacity deferral record")
