...
A candidate becomes ATTRIBUTABLE one step earlier still, when it is committed, so every generated ship brief requires the worker to run `bin/fm-commit-identity.sh` before its first commit; its library header owns the channel-capability contract, and a refusal or unobservable result blocks committing until identity configuration is repaired.
