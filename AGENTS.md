...
config/qualification-contract-dir  the capability-contract generation this home reads; LOCAL, gitignored; one line, absolute or relative to FM_HOME, naming the COMPLETE contract directory; absent = the tracked qualifications/contracts. An explicit FM_QUALIFICATION_CONTRACT_DIR outranks it and the tracked default is last; a binding that does not resolve, or that lacks a contract the tracked generation ships, is REFUSED rather than falling back (bin/fm-qualification-lib.sh; docs/configuration.md). Inherited by secondmate homes
