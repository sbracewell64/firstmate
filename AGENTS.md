...
state/               volatile runtime signals; gitignored
  ...
  <id>.capacity      durable record of a dispatch DEFERRED because no model meeting its floor had capacity, written only by bin/fm-capacity-retry.sh, which also resumes it automatically once the recorded retry condition is met; the same file's bound is counted by bin/fm-attempt.sh as deferrals=/deferral_budget=, spends no attempt, and reaches budget_exhausted rather than polling forever (docs/configuration.md "Capacity deferral record")
