...
Immediately before it selects a pool slot, `fm-spawn` computes one pre-reservation product joining this work's generation, its role/binding/resource path, current qualification, both bases and venue, and the SINGULAR owner of the candidate's mutation, and a refusal or a could-not-observe there allocates no slot, reservation, branch, worktree change, task metadata or worker at all; a candidate a live no-mistakes run still owns is a refusal rather than a race, and `bin/fm-role-path-lib.sh` owns the product while `bin/fm-route.sh role-path` reads it.
Treat that stop as the state to repair rather than an obstacle to bypass, and waive one named axis with `--allow-unpreflighted <REASON_CODE>` only on a current explicit captain instruction for that concrete dispatch, which is recorded on the task.
