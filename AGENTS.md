...
`bin/fm-harness-adapter.sh` and `bin/harnesses/<harness>.sh` own the per-harness wiring-artifact inventory that spawn excludes and teardown removes, plus which harness names are launchable versus supported for the primary session.
