...
state/               volatile runtime signals; gitignored
  ...
  model-observation.json  narrow mode-0600 record of what each model's LAST PROBE OBSERVED, three-valued; bin/fm-availability-lib.sh owns the type, the schema and every write, and validates the record on each read so a file that parses but is not valid refuses rather than reading as an empty exclusion set. bin/fm-model-verify.sh records what a probe observed and bin/fm-route.sh's supported release retires an UNAVAILABLE entry an operator explicitly overrides; nothing else writes it
