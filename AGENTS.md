...
loopspecs/           canonical LoopSpec registry, committed: schema.json (field contract), triggers.json (the sixteen-trigger register), terminal-states.json (the unified terminal-reason vocabulary and its total mapping from every source vocabulary, now GENERATED from semantics/state-families.json and never hand-edited), and one <id>.json per loop; bin/fm-loopspec.sh is their only interpreter (section 13)
semantics/           canonical state and seam semantic owner, committed: schema.json (field contract and the refused keys that keep this register from becoming a second decision surface), laws.json, state-families.json, reasons.json, identity.json, seams.json, inheritance-manifest.schema.json, manifests/<id>.json per load-bearing mechanism, census.json (the migration source of truth, explicitly not an owner), and generated/ projections; bin/fm-semantics.sh is their only interpreter and compiles every projection, and docs/state-seam-semantics.md owns the architecture. It describes vocabulary and legality only and holds no task state, schedule, queue, decision or runtime; everything it declares is DIAGNOSTIC until its adoption prerequisites pass
