...
A candidate becomes ATTRIBUTABLE one step earlier still, when it is committed, so `bin/fm-spawn.sh` binds the production commit identity through `bin/fm-commit-identity.sh` before any task it dispatches exists: it installs the publication identity policy's declared author and committer into the project and the repository the validation pipeline commits in, refuses the launch when that identity is missing, placeholder, ambiguous or unobservable, and reports a home that declares no policy as ungoverned rather than refusing it. The generated brief keeps its own call as projection, because that one runs in the worker's process and is the only check that sees the worker's own environment; a worker reporting either refusal is blocked on identity configuration rather than on the task.
