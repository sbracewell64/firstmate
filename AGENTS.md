...
A candidate becomes ATTRIBUTABLE one step earlier still, when it is committed; `bin/fm-spawn.sh` enforces the production commit identity admission owned by `bin/fm-commit-identity.sh`, and a worker reporting an identity refusal is blocked on identity configuration rather than on the task.
