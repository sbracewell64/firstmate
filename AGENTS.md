...
state/               volatile runtime signals; gitignored
  ...
  <id>.blockers      durable baseline of the upstream blockers the backlog records for this task and the disposition each was last observed in, written only by bin/fm-blocker-lib.sh and removed by teardown; it is what makes a declared wait's recheck DEPENDENCY-driven rather than timer-driven, so a hold whose named blocker has not moved stops waking firstmate on the hour while a blocker that moved, one that could not be read, and a refused dependency cycle all still do. An unobserved blocker never reads as unchanged, and this changes cadence only: the hold stays exactly as visible in the backlog, the fleet view, and the return catch-up
