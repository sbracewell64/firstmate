...
config/sol-control.json  optional Browser Sol control venue the outbound transport invariant addresses a request to; LOCAL, gitignored; absent leaves a waiting item's artifact state could-not-observe rather than clear, so it never reads as satisfied, and present it must also declare the `landing_domain` both merge chokepoints read; see docs/configuration.md "Browser Sol control venue"
config/publication-identity.json  optional per-venue governed publication identities, semantic work identities and policy generation; LOCAL, gitignored; absent means this home declares no publication governance, while its presence makes an unplaceable venue could-not-observe rather than ungoverned; see docs/configuration.md "Publication identity policy"
...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  outbound-artifacts/  durable correlation records joining an outbound request to its ruling, waiting item, and disposition; written only by bin/fm-outbound-artifact.sh, which owns their format, identity rule and crash-safe checkpointing.
                     ONE record serves both directions, so a ruling stays attributable to the request that asked for it.
  landing-authorizations/  one-use, head-bound authorities for the fleet's irreversible outward effects: LANDING, derived from those rulings and spent by the two landing chokepoints, CANDIDATE PUBLICATION, spent by bin/fm-publication-guard.sh before a remote moves, CUSTODY REPLICATION, spent by the same guard for a backup that grants nothing, and ATTESTATION EVIDENCE, spent by that guard for the exact notes-ref update without a semantic work identity.
                     bin/fm-landing-authorization-lib.sh owns their shared identity, lifecycle, record store, and intent-before-act spend sequence; the file name is narrower than its contents and docs/vocabulary-collisions.md rules that
  <id>/outbound-gate.json  the typed declaration that this item is waiting on an outbound artifact, naming its exact binding and backing request; the canonical alternative to firstmate recognising the wait from hold prose. Written through bin/fm-outbound-artifact.sh's declare, which REFUSES to enter a wait no live request backs, so a bare wait is prevented where it would be created rather than reported by a later sweep
...
state/               volatile runtime signals; gitignored
  ...
  <id>.meta          written by fm-spawn: window=, endpoint_task_id=, worktree=, project=, harness=, model=, effort=, mode=, yolo= (whose two values and the one comparison that reads them are owned by bin/fm-autonomy-lib.sh, which every producer validates against and every consumer reads through), tasktmp=, optional profile= when a named invocation profile resolved, and (ship and scout) attempt=/attempt_budget= copied from the durable attempt record, where an absent attempt= reads as attempt 1; the task identity axes role=, deliverable=, and stage=, plus the deprecated kind= they replace, owned with their derivation and retirement by bin/fm-task-axis-lib.sh and docs/vocabulary-collisions.md; a ship or scout also records the task's two base references as slot_base=, contribution_target=, and base_state=, and a ship additionally records the venue that target names as contribution_venue=/contribution_venue_url=, which bin/fm-pr-check.sh refuses a contradicting pull request against after resolving any ssh host alias in that url, plus publication_repo= naming the candidate repository authorized to receive its attestation note, and the policy ROLE that venue's attestation gate is read through as policy_ref= (the ref that owns policy there, placed in the venue's own namespace) and policy_generation= (the commit it resolved to at dispatch), which are a separate axis from contribution_target= even where they name the same commit and which bin/fm-attest.sh re-resolves at the effect boundary rather than trusting as recorded, an absent policy_ref= leaving currency could-not-observe rather than licensing the venue's HEAD as a substitute (bin/fm-task-base-lib.sh; docs/no-mistakes-attestation.md); a task dispatch also records the agent-justification fields reasoning_required=, reason_code=, capability_floor=, escalation_policy=, plus tooling_gap_item= for a TOOLING_GAP dispatch (bin/fm-reasoning-lib.sh, section 7), and route= with route_policy_digest= naming the route the dispatch was checked against and the exact policy surface that checked it, assigned once and never rewritten by a later layer (bin/fm-route-lib.sh), plus capacity_observed=/capacity_evidence= naming the three-valued provider-capacity verdict that dispatch was admitted against, written for every ship and scout dispatch in a home with routed pools including when it could not be observed (bin/fm-capacity-lib.sh), while a secondmate provisioning spawn records none of them and an absent field reads as unknown rather than as justified reasoning; an optional traceparent= only when trace context is enabled (docs/configuration.md "Trace context propagation"); role=secondmate also records home= and projects=, plus remote_host=/remote_root=/remote_backend=/remote_herdr_session=/remote_target= for a remote route; a non-default runtime backend records further backend-specific fields (docs/configuration.md "Runtime backend"; bin/fm-backend.sh, section 8); fm-pr-check, including through fm-pr-merge, records one canonical pr= and the forge's pr_head= when available (GitHub pull requests and GitLab merge requests; docs/gitlab-merge-watch.md); fm-pr-merge records merge_verification= plus merge_verified_head= for the head it re-verified, or merge_verification=override plus merge_waived_check= naming the single check an explicit override waived, and landing_authorization= naming either the one-use landing authority it spent or the literal not-applicable, so a landing proven outside the declared governed domain is recorded rather than merely unmentioned (bin/fm-landing-seam-lib.sh, section 7); fm-x-link appends x_request=, x_request_ts=, x_followups=, and optional x_platform=/x_reply_max_chars= for an X-mode-originated task (section 14)
  <id>.attempt       durable attempt count and retry budget for the task id (attempt=, attempt_budget=, failures=, terminal=), plus the capacity-deferral bound that shares this record (deferrals=, deferral_budget=, defer_signature=, defer_stagnant=), owned by bin/fm-attempt.sh; spent only by a ship or scout spawn that follows a RECORDED FAILURE, while a spawn after a dead runtime or husk continues the attempt already open, and retired by an ordinary teardown but kept under --force, so a retry decision is arithmetic rather than a judgment; the SAME record carries the task's EXECUTION lineage (execution=, execution_id=, execution_binding=, execution_effort=, execution_dispatch=, execution_head=), which names the worker incarnation now running the lane and moves only through that script's `replace`, never with the attempt count
  <id>.lineage       append-only execution-attempt ledger for the task id, written only by bin/fm-attempt.sh: one typed `fm-execution-lineage.v1` line per execution opened or closed, carrying the binding that produced the work under it. Nothing rewrites it, which is what keeps a superseded worker's evidence attributed to the worker that produced it; it retires and survives teardown on exactly the same terms as <id>.attempt
  <id>.capacity      durable record of a dispatch DEFERRED because no model meeting its floor had capacity, written only by bin/fm-capacity-retry.sh, which also resumes it automatically once the recorded retry condition is met; its bound is counted in <id>.attempt by bin/fm-attempt.sh as deferrals=/deferral_budget=, spends no attempt, and reaches budget_exhausted when that bound is spent, or blocked_by_evidence_integrity when the count itself could not be written, rather than polling forever - the two are different terminal states because a bound that was reached and a bound that was never enforceable are different facts (loopspecs/terminal-states.json, source capacity-wait) (docs/configuration.md "Capacity deferral record")
  <id>.blockers      durable blocker-disposition baseline written by bin/fm-blocker-lib.sh and removed by teardown; see docs/architecture.md "Event-driven supervision" for the dependency-driven pause re-evaluation contract
...
3. **Wake queue** - when locked, drains the durable wake queue and prints the raw records prominently as this turn's first work queue; a bounded, clearly labeled historical status-event annotation may follow a valid `signal` record but never replaces it or current-state reconciliation, and a lapsed watcher chain still surfaces here via the same guard alarm.
   Every locked drain also prints the complete fleet-wide `OPEN DECISIONS` section when durable decision records remain open, including when the queue itself is empty, each entry carrying one disposition; reconcile those entries before continuing.
   A `CNO_DECISION_UNIVERSE` line means that listing is INCOMPLETE and is an instrument defect rather than more captain decisions; when the wake presentation bound caused it, read the complete set from `bin/fm-fleet-snapshot.sh --json`, otherwise repair the named unreadable status log, before stating what needs the captain.
...
A spawn allocates only a demonstrably empty pool slot and skips any that still holds live work, so it refuses before allocating only when the pool offers no empty slot it may hand this task; that refusal names each slot, its evidence, and the apparent owner, and is a stop-and-investigate result rather than an obstacle to bypass.
When a trunk repair is queued and the pool has no free slot, `bin/fm-slot-reservation.sh` opens a slot reservation - the fleet's other `reservation` is admission control's, ruled apart in `docs/vocabulary-collisions.md` - that holds the NEXT slot to free for that one task, and the guard then refuses every other dispatch that slot; it creates no capacity, never preempts a running lane, and expires, so read that script's header before opening, releasing, or explaining one.
...
Where this home's dispatch config carries routed pools, every ship and scout spawn also names the route it claims with `--route` and the model it will run, and `fm-spawn` refuses a model outside that route's pool, below its floor, held by the availability record, or with no recorded evidence for an axis the floor declares; substitute a failed model only from the same pool in order, and treat a pool with no policy-eligible candidate as a stop rather than a reason to reach outside it or lower the floor (`bin/fm-route.sh`, `bin/fm-route-lib.sh`).
...
A lane ALREADY UNDER WAY whose provider window closes is a different case and never a new dispatch: it keeps its slot, its worktree, its branch and its work, and only its EXECUTION ATTEMPT is replaced.
`bin/fm-attempt.sh replace` is the only thing that sanctions one, and `bin/fm-spawn.sh --succeed-execution` the only thing that launches it; that pair asks the allocator for nothing, so a full pool never blocks it.
The replacement spends no attempt, because a provider that ran out of window did not fail the work, and the successor is a NEW producer, so evidence made before it keeps the binding that made it.
Every condition it needs comes from the owner that already answers it, and one it could not establish refuses rather than proceeding; read that script's header before opening, explaining, or working around a refusal.
...
For a publication, an active hold, a revoked or superseded ruling, an outbound state no landed vocabulary declares, a governed head no ruling has reviewed, a reviewer the role qualification register does not currently record as qualified and assignment-distinct, a placeholder or unmapped governed identity, a second actionable candidate for the same work, a stale generation, a moved remote tip, and a replay each refuse before the remote moves; for a custody replication, a forcing act, a protected ref, any ref but the work's own, an unclean worktree, a head that is not the one checked out, and an occupied ref each refuse the same way.
A remote already at that head is a typed no-effect that consumes nothing, and an ungoverned publication proceeds and says so.
`bin/fm-publication-guard.sh project` is the read-only projection of which explicit state a candidate is in - `local-only`, `custody-replicated`, `review-published`, `publication-qualified`, `landing-authorized`, `landed` - where none implies the next, and its exit status answers whether the slot holding that candidate may be reclaimed.
That guard covers firstmate's own paths only: a direct push, a provider UI, and no-mistakes' own push step reach a remote without it, and server-side protection remains the separate defence for those.
...
Run `bin/fm-pr-check.sh <id> <PR url>` - it records `pr=` and the forge's `pr_head=` when available in the task's live meta or released-task landing record, arms the watcher's PR poll, and then invokes the attestation-publication chokepoint described in `docs/no-mistakes-attestation.md`.
