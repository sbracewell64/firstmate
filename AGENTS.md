...
state/               volatile runtime signals; gitignored
  ...
  model-price.json   the last price each routed model was observed at, straight from the provider, written only by bin/fm-model-verify.sh through bin/fm-model-price-lib.sh; cost only, deliberately a separate file from model-health.json because reachability and cost are independent axes, and evidence rather than authority - the dispatch gate re-observes and never reads this
