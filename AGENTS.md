...
2. For `stale:`, inspect the recorded endpoint and load `stuck-crewmate-recovery` for a stopped, looping, confused, or unresponsive worker; a deep-inspection reason also requires current-state and validation-log inspection - this wake is the worker sense of `stale`, and the evidence sense is always written `verification-stale`, ruled apart in `docs/vocabulary-collisions.md`.
