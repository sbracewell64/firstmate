...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  landing-authorizations/  one-use, head-bound authorities for the fleet's two irreversible outward effects: LANDING, derived from those rulings and spent by the two landing chokepoints, and CANDIDATE PUBLICATION, spent by bin/fm-publication-guard.sh before a remote moves.
...
A candidate reaches the outside world one step earlier, when it is PUSHED, so every remote-changing publication firstmate itself performs passes `bin/fm-publication-guard.sh` first through the single wiring in `bin/fm-publication-seam-lib.sh`, which recompiles permission at the moment of use and spends a one-use authority bound to the exact ref, head, tree, remote tip and ruling-and-policy generation; an active hold, a revoked or superseded ruling, a placeholder or unmapped governed identity, a second actionable candidate for the same work, a stale generation, a moved remote tip, and a replay each refuse before the remote moves, a remote already at that head is a typed no-effect that consumes nothing, and an ungoverned publication proceeds and says so.
