...
[
 ...
 {"compensation":"unenforced_commitments","status":"owned",
  "owner":"bin/fm-commitment-register.sh, relayed by bin/fm-bootstrap.sh at every session start",
  "note":"whether a recorded commitment is real yet is computed from its probe on every read; a commitment nobody enforces cannot read as protection, and one that becomes real retires without a hand edit"},
 {"compensation":"outbound_artifact_existence","status":"owned",
  "owner":"bin/fm-outbound-artifact.sh check, relayed by bin/fm-bootstrap.sh at every session start",
  "note":"whether the artifact an item is waiting on actually exists is observed on the forge against the item's exact head; waiting with no applicable artifact is a transport defect rather than an external wait, and firstmate may not call such an item legitimately blocked"},
