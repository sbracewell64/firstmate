...
# and a branch cut from another. Ship only: a scout has no branch to misplace,
# and its worktree is placed at the slot base whatever its brief happens to say.
if [ "$KIND" = ship ] && [ "$BASE_STATE" != coincident ]; then
  ...
  if [ -z "$BRIEF_BASE" ]; then
    echo "warning: $BRIEF records no base contract line but this task's read and contribution bases differ (${SLOT_BASE:0:12} vs ${CONTRIB_TARGET:-unresolved}); re-scaffold the brief with --slot-base/--contribution-target so the worker is told which commit to read and which to branch from" >&2
  else
