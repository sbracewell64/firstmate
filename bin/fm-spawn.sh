...
EOF
    if [ -n "$ROUTE_FLOOR_ID" ] && [ "${#QUALIFICATION_MODELS[@]}" -gt 0 ]; then
      QUALIFICATION_LINES=$(fm_qualification_route_lines \
        "$(fm_route_config_path "$CONFIG")" "$ROUTE_FLOOR_ID" \
...
if [ "$KIND" = secondmate ]; then
  ...
  # FM_SESSION_ORIGIN_ID is cleared with the overrides because nothing on this
  # path ever owns an origin record: the id belongs to the one unattended start
  # that wrote it, and a secondmate primary inheriting it from the pane's parent
  # would announce itself unattended and claim a record it does not own. Absence
  # must keep meaning captain-started (bin/fm-unattended-session.sh).
  LAUNCH="FM_ROOT_OVERRIDE= FM_STATE_OVERRIDE= FM_DATA_OVERRIDE= FM_PROJECTS_OVERRIDE= FM_CONFIG_OVERRIDE= FM_SESSION_ORIGIN_ID= FM_PUBLIC_FOLLOWUP_PRIMARY_HOME=$sq_primary_home FM_HOME=$sq_home FM_TRACE_CONTEXT=$SPAWN_TRACE_EFFECTIVE FM_SUPERVISION_MODEL=$supervision_model $LAUNCH"
fi
