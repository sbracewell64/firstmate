...
family_for_basename() {
  case "$1" in
    ...
    fm-teardown-endpoint-safety.test.sh)
      printf '%s\n' backend-dispatch
      ;;
    fm-attest.test.sh|fm-pr-check-security.test.sh|fm-pr-merge.test.sh|\
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-timeout-lib.sh)
      ...
      # snapshots, the vendor auth probe, and every bounded `axi status` read
      # through bin/fm-nm-run-lib.sh all depend on it.
      printf '%s\n' session-bootstrap
      printf '%s\n' snapshot-bearings
      printf '%s\n' pure-contract-unit
      printf '%s\n' pr-forge
