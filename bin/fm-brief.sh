...
When the pipeline reports CI green (the CI-ready return point - do not wait for it to keep monitoring in the background until merge), resolve and validate the forge-observed final PR head and publish its head-bound evidence in this worktree with one command:
   ...
   \`final_head="\$(gh pr view {url} --json headRefOid --jq .headRefOid)" && [[ "\$final_head" =~ ^[0-9a-fA-F]{40}\$ ]] && . "$FM_ROOT/bin/fm-backend.sh" && "$FM_ROOT/bin/fm-attest.sh" write --only-if-required --policy-meta "$FM_ROOT/state/$ID.meta" --publish-repo "\$(fm_meta_get "$FM_ROOT/state/$ID.meta" publication_repo)" --publish-notes-ref refs/notes/no-mistakes --expect-head "\$final_head"\`
