...
- A read of a remote collection whose result can reach "no X exists", "nothing pending", or "already handled" goes through [`bin/fm-control-read.sh`](bin/fm-control-read.sh), which enumerates every page and refuses a negative it cannot support; `bin/fm-retrieval-check.sh --check` is the CI gate and requires an inline classification with a reason for any read that stays direct.
  [`docs/verification/control-plane-retrieval.md`](docs/verification/control-plane-retrieval.md) records the audit, the measured mechanism facts, and the residuals retained on purpose.
...
The stock macOS Bash snapshot and Bearings checks derive expected test identities from each suite's declarations, so legitimate additions do not require a maintained count while omitted invocations fail.
