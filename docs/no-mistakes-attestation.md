...
`write` refuses with `no-run-record`, `run-record-unreadable`, `run-record-unparsed`, `run-record-no-head`, `run-covers-another-branch`, `run-head-unavailable`, `run-covers-another-head`, `expected-head-mismatch`, `run-incomplete`, `publication-target-unbound`, `publication-target-mismatch`, `publication-notes-ref-unbound` or `publication-notes-ref-mismatch`.
