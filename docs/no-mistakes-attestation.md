...
The remote-changing notes-ref push passes through [`bin/fm-publication-guard.sh`](../bin/fm-publication-guard.sh), so a governed publication must satisfy the publication identity policy and exact-candidate ruling described in [`configuration.md`](configuration.md#publication-identity-policy-configpublication-identityjson) before the ref can move.
