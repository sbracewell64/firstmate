...
- `herdr` calls `herdr notification show` outside the supervised pane.
  That command exits 0 even when it showed nothing, so the channel reads the reply payload and treats an explicit `"shown": false` - such as Herdr's default-off `[ui.toast] delivery` answering `"reason": "disabled"` - as a channel failure, logging the reported reason; a reply with no `shown` field keeps the exit-status verdict.
...
A missing binary, a non-zero exit, or a reply proving the notification was not shown logs a warning and continues to the next channel without crashing the daemon loop.
