...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling, a monitored pull request the forge reports as conflicting, or an X-mode mention, stale panes whose crew is neither provably working nor settled in a terminal state, whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, declared external waits re-evaluated past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
That re-evaluation is dependency-driven rather than timer-driven wherever the backlog records an exact upstream blocker: `FM_PAUSE_RESURFACE_SECS` becomes the sampling bound, and the wake happens only when that blocker moved, could not be observed, or sits in a refused dependency cycle, while a wait with no recorded blocker keeps the plain timer.
[`bin/fm-blocker-lib.sh`](../bin/fm-blocker-lib.sh) owns that verdict, the durable `state/<id>.blockers` baseline it compares against, and the cycle refusal.
In away mode the pause recheck is additionally limited to waits that can change without the captain; the `/afk` skill owns that rule.
...
A descendant that merely exists is never evidence, so a crew whose child is hung, dead, or absent still surfaces on the unchanged schedule, and a definite parked, done, or blocked run-step verdict is never overridden by the process tree.
A run that has ENDED - rejected, deliberately cancelled, or broken without a verdict - is the one class those crew-level sources answer for rather than being overridden by, because that verdict describes a run and never observed the crew, which is then free to start a replacement run.
`bin/fm-crew-state.sh` therefore records the crew's own turn signal and its agent liveness alongside such a verdict without changing the verdict, source, or precedence, and `bin/fm-classify-lib.sh` keeps all three answers apart.
A live turn whose agent the backend probe establishes as alive absorbs, and so do advancing descendants on their own; an agent established dead or missing, an observed-idle crew, or a gone endpoint escalates on the unchanged schedule; and an agent whose liveness cannot be established, including a busy turn record nothing corroborates, escalates saying it could not be observed, as does turn evidence that expired, was never written, or could not be read.
A live shell is not a live agent, which is why the turn signal is never trusted alone there: a busy record is trusted until `FM_BUSY_MAX_BUSY_AGE_SECS` from a timestamp stamped when the turn opened, so a worker killed behind a surviving shell would otherwise read as working for up to an hour on exactly the lane the alarm exists for.
The corroborating readings are therefore both re-derived per read rather than cached: `fm_backend_agent_state` in `bin/fm-backend.sh`, which is backend-native and bound to the crew's recorded endpoint, and the identity-bound descendant-CPU probe, which measures advancement.
Neither identifies an agent by searching process names or command lines, because such a pattern has matched the wrong process when it appeared in a worker's brief or in the searching process's own command line.
Narrowing that third value into silence would trade the measured false-escalation noise for missing exactly the wedge the alarm exists to catch, so it escalates on the same schedule as an idle crew and only names itself.
A crew that declares `paused:` for a known external wait is separately absorbed while idle and re-evaluated only on the longer pause cadence, rather than being treated as a possible wedge.
That evaluation re-surfaces a wait with no recorded blocker, an observed blocker movement, an unreadable blocker, or a refused dependency cycle, but an observed unchanged blocker remains absorbed.
...
Away-mode wedge aging asks the same positive-evidence question at one point only, the moment an aged marker is about to escalate: a crew that is provably working has its marker refreshed rather than escalated, so proven progress that never reaches the pane cannot raise a possible-wedge alarm every threshold, while a crew without that evidence still escalates on the unchanged schedule.
Refreshing rather than dropping is what keeps that bounded, and it fails toward noise rather than silence: remaining silent requires the working verdict to be re-earned every threshold, so a crew that freezes, or a reader that stops answering, ages out and escalates without anything having to notice and re-arm it.
...
That state read is bounded per crew per threshold, which does not bound one housekeeping pass, so a pass makes at most `FM_STALE_WORKING_GATE_READS` of them and leaves any remaining aged marker untouched for the next pass, which delays such an escalation rather than dropping it.
For a burst of `D` markers due at once the delay is one pass per full budget of markers ahead of it, so the last waits `ceil(D/B) - 1` passes of `FM_HOUSEKEEPING_TICK` at a budget of `B`: ten due at the default budget of 3 and tick of 15s put the last escalation `ceil(10/3) - 1 = 3` ticks, or 45s, late rather than within one tick.
That bound assumes the due set drains, and it stops holding under sustained fleet-wide staleness: the gate does `B` reads per `FM_HOUSEKEEPING_TICK` while each continuously-stale crew needs one read per `FM_STALE_ESCALATE_SECS`, so the backlog grows without bound, and the delay with it, beyond `B * FM_STALE_ESCALATE_SECS / FM_HOUSEKEEPING_TICK` simultaneously-stale crews - roughly 48 at the defaults.
...
A declared external wait trades that silence for one bounded evaluation per pause window, while its structured hold remains visible even when an observed unchanged blocker suppresses the wake.
Crew status files are append-only wake-event logs, not current-state fields.
Each event is written as a typed `fm-status-event.v1` envelope carrying `verb`, `key`, `phase`, repeatable `evidence` references and one bounded `summary`; `bin/fm-status-event-lib.sh` owns that format and `bin/fm-classify-lib.sh` owns what the verb means.
For an eligible PR-capable ship task in away mode, a valid `done` event at `phase=ready` with exactly one canonical PR/MR evidence reference is registered by `bin/fm-supervise-daemon.sh` through `bin/fm-pr-check.sh` before seen-status dedupe, but it never replaces a different recorded PR identity; present mode preserves the normal firstmate handoff.
Classification reads the verb and never the summary, so a crew's own wording cannot change how its event is triaged, and the prose form remains readable while the fleet migrates.
What a task is waiting on is derived rather than declared: `status_event_blocking_on` combines the verb, the keyed open-decision fold and `bin/fm-crew-state.sh`'s typed verdict, an event that tries to carry `blocking_on` itself is refused whole, and proof that a run is advancing outranks a stale claim of being blocked.
...
Enumerating which decisions are open is a cheap read of durable records and deciding whether one's pinned criterion is met is expensive verification, so the fold executes no registered probe unless a caller grants it an explicit probe budget, and a criterion with no stored observation is carried on its entry as could-not-observe rather than removing the entry or stalling the listing.
Every enumerated entry also carries exactly one disposition from the closed vocabulary `FM_DECISION_DISPOSITION_VOCABULARY` owns, `CNO_DECISION_SUBJECT` included, and a presentation that cannot show the whole universe reports `CNO_DECISION_UNIVERSE` with both counts as an instrument defect instead of trailing a quiet omission footnote.
...
A resolution for a key whose decision file pins a probe is not accepted while that probe fails or cannot be observed, so a fix reported applied cannot close a criterion nothing established; the `ask-user-authority` skill owns the pinned probe block and `bin/fm-commitment-register.sh` interprets it.
...
Both of those reads are repository-scoped, and the repository is not the universe: no-mistakes registers every managed run worktree as its own repository, so a run created from inside one is absent from the enclosing checkout's `axi status` and from `no-mistakes runs` alike, which is how a live lane once read as quiescence.
`bin/fm-nm-census.sh` closes that with one WAL-consistent read transaction over the no-mistakes state database that enumerates every registered repository, nested identities included, and joins every potentially candidate-owning run exactly once to firstmate work or classifies it unrelated, ambiguous, or could-not-observe.
no-mistakes' own database and AXI surface stay the run-state authority and firstmate adds no second one; what firstmate owns is the cross-system join, and the crew-state reader, teardown's run abort, and the fleet snapshot all consume that same census rather than re-deriving it.
Its verdict is weakest-wins and only `OBSERVED_QUIESCENT` is quiescence, with `universe.non_vacuous` separating a global zero taken over a populated universe from one taken over an empty database; `docs/verification/no-mistakes-run-census.md` records the measured generation and the limits.
...
Captain relevance is decided on the verb alone as the classification contract above states, so a nonterminal progress verb never escalates whatever its prose says and a bare verbless legacy line such as `merged` no longer escalates; free-text prose matching survives only where a home opts into a custom `FM_CAPTAIN_RE` vocabulary, and nonterminal verbs stay excluded even there.
The always-on watcher also uses that library's absorb classification on no-verb signals and first-sighting stale panes before status-log terminality is trusted, while the daemon maintains distinct wedge and declared-pause evaluation cadences.
...
In away mode, seen-status dedupe does not clear possible-wedge aging for nonterminal progress, so housekeeping still re-escalates an unchanged idle pane at the configured bound unless the provably-working refresh above applies at that escalation point.
...
The daemon escalates captain-relevant events, plus any declared-pause evaluation whose blocker verdict requires a wake, as one batched, single-line digest using the canonical `away-supervisor` kind from `bin/fm-operational-input.sh` so firstmate can distinguish it structurally from real messages.
...
The separate descendant-CPU liveness probe described above may supply positive evidence to watcher absorb classification only for a semantic read that leaves the crew's own liveness open: one the reader could not decide, or one that decided about a run that ended and therefore never observed the crew.
`bin/fm-classify-lib.sh` owns that membership in one place, so the watcher's stale detection and its absorb classification cannot disagree about which verdicts the process tree may answer for.
On the run-ended read the busy record is additionally never trusted alone: absorbing needs the crew's agent established alive by the backend probe, because the record's freshness bound is measured from when the turn opened and a shell can outlive the agent that opened it.
...
Unsupported effort values are still recorded in task meta when passed to `fm-spawn.sh`, but the launch template omits any effort flag that the selected harness does not accept, subject to the fail-closed Luna Max exception owned by the dispatch-profile contract.
...
Verification reads the head, mergeability, review decision, and check results in one GraphQL request at merge time rather than trusting a recorded value, so a head that went red after an earlier check is still caught and every state-based refusal names the exact head it evaluated once GitHub supplies a readable head.
Whether that head is green is not the merge gate's own judgment: [`bin/fm-verify-lib.sh`](../bin/fm-verify-lib.sh) is the single owner of that question, and its header owns the query, the refusal of results attached to a superseded commit or reported but not returned, and the reduction of repeated executions to the one current attempt per check, without which a single re-triggered check would leave its earlier run attached to the head forever.
`bin/fm-pr-merge.sh` consumes that verdict and refuses any label but passing, supplying only mergeability, the review decision, the waiver arithmetic, and its own refusal wording.
The guarded merge path rechecks the pull request head and all required merge conditions immediately before requesting the forge merge, but the forge call does not expose an atomic expected-head precondition.
The Captain explicitly accepts the resulting narrow post-check race as residual risk; that acceptance does not relax the green-head, mergeability, review, or explicit-waiver protections.
...
[`bin/fm-pr-merge.sh`](../bin/fm-pr-merge.sh)'s header owns the full refusal list and the `--allow-unverified <check-name>` override, which is never inferred, waives exactly the one named check while every other refusal still stops the merge, and records that check's name in the task's metadata so the record says what was waived rather than only that something was.
When a live Browser Sol review request governs the landing, both `bin/fm-pr-merge.sh` and `bin/fm-merge-local.sh` additionally run their merge command inside a one-use, head-bound authorization spend that composes with the guards above rather than replacing any of them; [`bin/fm-landing-seam-lib.sh`](../bin/fm-landing-seam-lib.sh) owns that applicability question and the spend sequence, and [configuration.md "Browser Sol control venue"](configuration.md#browser-sol-control-venue-configsol-controljson) owns the venue that makes a landing governed.
...
The in-flight condition has one deliberately composed answer: a run-less lane whose endpoint is gone reads `unknown` from `bin/fm-crew-state.sh` by design, and dead worktree custody, that endpoint-absent unknown read from its structured output, and no attributed run together establish observed quiescence, recorded on the lineage line that closes the predecessor, while every other unknown still refuses ([`verification/execution-attempt-replacement.md`](verification/execution-attempt-replacement.md)).
