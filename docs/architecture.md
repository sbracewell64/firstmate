...
PR-based task merges go through `bin/fm-pr-merge.sh`, which pre-checks the pull request's current head, records `pr=` and any available `pr_head=` through `bin/fm-pr-check.sh`, and then authoritatively re-verifies immediately before recording the verified head and calling `gh-axi pr merge`.
...
Verification reads the head, mergeability, review decision, and check rollup in one `gh pr view` call at merge time rather than trusting a recorded value, so a head that went red after an earlier check is still caught and every refusal names the exact head it evaluated.
