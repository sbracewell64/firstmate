...
`no-mistakes` tasks run the full validation pipeline, `direct-PR` tasks open PRs without that pipeline, and `local-only` tasks stay local until firstmate performs a guarded fast-forward merge under the compiled landing authority.
Each task's mode and `yolo` posture are firstmate's decision at intake and are passed explicitly to `bin/fm-brief.sh`, `bin/fm-spawn.sh`, and `bin/fm-reflag.sh`, which refuse a ship task that does not carry them; the recorded `yolo` value is a dispatch snapshot, while `bin/fm-autonomy-lib.sh` resolves the effective posture from the current registry whenever authority is consumed.
...
`bin/fm-project-mode.sh` remains the one registry parser for both the effective-autonomy resolver and the mechanical consumers that have no task in hand: fleet sync's `local-only` skip and home seeding's refusal and no-mistakes initialization.
...
Both `bin/fm-pr-merge.sh` and `bin/fm-merge-local.sh` compile whether landing authority is delegated or captain-reserved from typed durable sources, and when a live Browser Sol review request governs the landing they additionally run their merge command inside a one-use, head-bound authorization spend that composes with the guards above rather than replacing any of them; [`bin/fm-landing-seam-lib.sh`](../bin/fm-landing-seam-lib.sh) owns both authority questions and the spend sequence, and [configuration.md "Browser Sol control venue"](configuration.md#browser-sol-control-venue-configsol-controljson) owns the venue that makes a landing governed.
