...
For candidates inside the declared Browser Sol landing domain, both `bin/fm-pr-merge.sh` and `bin/fm-merge-local.sh` additionally require a live request covering the exact head and run their merge command inside its one-use, head-bound authorization spend, composing with the guards above rather than replacing any of them; [`bin/fm-landing-seam-lib.sh`](../bin/fm-landing-seam-lib.sh) owns that applicability question and the spend sequence, and [configuration.md "Browser Sol control venue"](configuration.md#browser-sol-control-venue-configsol-controljson) owns the operator-facing declaration.
