...
Away-mode wedge aging asks the same positive-evidence question before it alarms: a stale pane whose crew is provably working is dropped from wedge aging, at first sight and again immediately before any escalation, so proven progress that never reaches the pane cannot raise a possible-wedge alarm on every threshold; a crew without that evidence still escalates on the unchanged schedule.
That drop is re-armed rather than permanent, because it clears the watcher's per-hash stale marker along with the daemon's own: the unchanged pane is surfaced again, the working verdict is re-read rather than trusted once, and a run that freezes after proving itself resumes wedge aging on the same schedule.
