...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling, a monitored pull request the forge reports as conflicting, or an X-mode mention, stale panes whose crew is neither provably working nor settled in a terminal state, whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, declared external waits after `FM_PAUSE_RESURFACE_SECS` except unchanged capacity observations whose deterministic owner retains the cadence, and heartbeat backstop hits.
In away mode that pause check is limited to waits that can change without the captain; the `/afk` skill owns that rule.
...
A declared external wait is inspected on every bounded pause window, and a generic wait still wakes firstmate for the scheduled recheck because no other observer is guaranteed.
A capacity wait points to its deterministic owner's evidence with a typed `capacity:` reference or active capacity record, so only its first or changed durable pause event wakes firstmate and an unchanged observation stays visible internally without repeatedly interrupting the captain.
The same task-keyed capacity-notification marker is shared across normal and away supervision, so switching modes cannot replay the unchanged event.
