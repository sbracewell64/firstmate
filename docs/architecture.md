...
PR-based task merges go through `bin/fm-pr-merge.sh`, which re-verifies the pull request's current head and records `pr=` and any available `pr_head=` through `bin/fm-pr-check.sh` before calling `gh-axi pr merge`.
