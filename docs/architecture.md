...
A decision's semantic lifecycle and the freshness of the observation that resolved it are separate axes: a resolution whose criterion could not be verified now stays semantically resolved and is enumerated as `CNO_DECISION_VERIFICATION`, never reopened as a captain-owed decision, and `bin/fm-classify-lib.sh`'s "FRESHNESS IS NOT A REOPEN" owns that rule with `decision_entry_is_ruling_owed` as the one predicate consumers ask.
