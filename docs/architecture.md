...
Away-mode wedge aging asks the same positive-evidence question before it alarms: a stale pane whose crew is provably working is dropped from wedge aging outright, at first sight and again immediately before any escalation, so proven progress that never reaches the pane cannot raise a possible-wedge alarm on every threshold; a crew without that evidence still escalates on the unchanged schedule.
