...
That request also asks which commit GitHub attached those results to and how many it holds, because results belonging to a superseded commit and results GitHub reported but did not return are both refused rather than counted toward the head being merged.
A head accumulates every execution ever run against it, so executions are reduced to one verdict per check - keyed by owning workflow and name, decided by the attempt with the highest run ID - before anything is counted; without that reduction a single re-triggered check would leave its earlier run attached to the head forever and no later pass could ever clear it.
