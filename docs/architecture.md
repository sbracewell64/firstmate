...
A valid `done` event at `phase=ready` with exactly one canonical PR/MR evidence reference is registered by `bin/fm-supervise-daemon.sh` through `bin/fm-pr-check.sh` before away-mode seen-status dedupe, while present mode preserves the normal firstmate handoff.
