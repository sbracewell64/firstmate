...
`bin/fm-crew-state.sh` therefore records the crew's own turn signal alongside such a verdict without changing the verdict, source, or precedence, and `bin/fm-classify-lib.sh` keeps all three answers apart: a live turn or advancing descendants absorb, an observed-idle crew or a gone endpoint escalates on the unchanged schedule, and turn evidence that expired, was never written, or could not be read escalates saying it could not be observed.
