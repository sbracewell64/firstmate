...
FM_PAUSE_RESURFACE_SECS=3600       # seconds before the watcher or away-mode daemon inspects an idle declared external wait; generic waits notify each window, capacity waits with typed evidence or an active capacity record notify only for a first or changed durable event, and the away-mode daemon skips captain-gated waits
