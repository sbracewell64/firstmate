...
FM_PARK_SCAN_SECS=120              # seconds between bounded park sweeps for a run parked at a decision gate its worker has not answered
FM_PARK_SCAN_MAX=3                 # ship tasks read per park sweep, rotated by a persisted cursor so a larger fleet is still fully covered; 0 disables the sweep
FM_SLOW_POLL_SECS=                 # seconds of watcher classification work in one poll before a saturation line is recorded; unset derives two thirds of FM_POLL, 0 records every poll
FM_WATCH_TRIAGE_LOG_MAX_BYTES=262144   # size cap for the watcher and wake-drain supervision debug log
