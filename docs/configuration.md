...
- `qualifications/records/<id>.json` is one observation of one binding against one contract, keyed by contract, fully qualified model, harness, harness version and native effort; provider is implied by the required fully qualified model and remains recorded as binding evidence. A different harness, harness version or effort band is a different thing that was never observed, not a stale version of one that was, and it is reported as a near miss rather than as an absence.
