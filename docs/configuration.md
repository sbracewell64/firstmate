...
The `author` and `committer` axes have a second consumer one step earlier in the same lifecycle: [`bin/fm-spawn.sh`](../bin/fm-spawn.sh) runs [`bin/fm-commit-identity.sh`](../bin/fm-commit-identity.sh) as a launch precondition, before allocating a task slot or backend endpoint, to validate and re-observe the finalized identity in the current project worktree and required pipeline repository.
After allocation it installs the same validated identity in the new isolated task worktree before worker execution.
