...
Those subjects are NOT the bytes shipped at this head: the review-round fixes recorded under "Nine review findings, and the round that closed them" edited all three of them, so the campaign is invalidated by design and awaits re-measurement. The campaign-artifact control refuses until it is re-measured, which is that invalidation being visible rather than assumed.
