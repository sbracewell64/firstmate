...
Every registry entry also owns its experiment: an input path reordered alone before recompiling, a compiled-body path reordered alone before digest and identity recomputation, or an exemption anchored to a structural contract maximum of one element.
The live registry has no structurally single-element array path, so its exemption set is empty; a fixture that supplies fewer than two distinct values for a reorderable path is a fixture defect, not evidence of vacuity.
