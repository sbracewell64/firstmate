...
Those subjects are NOT all the bytes shipped at this head: the review-round fixes recorded under "Nine review findings, and the round that closed them" edited two of the three - `bin/fm-review-envelope-lib.sh` and `tests/fm-review-envelope.test.sh`, with `bin/fm-review-envelope.sh` untouched and still hashing to its recorded digest - so the campaign is invalidated by design and awaits re-measurement. The campaign-artifact control refuses until it is re-measured, which is that invalidation being visible rather than assumed.
...
The deferral that stood here carried the stale subject digests it was pending on, and the count-drift control refused it the moment the artifact matched every shipped subject again - which is how this number came to be stated rather than left deferred indefinitely. A reason without a condition is just a reason.
...
| a base that falls behind the trunk refuses | the base-to-trunk distance is no longer compared with policy | `test_a_base_that_falls_behind_the_trunk_refuses` | `not ok - the refusal must name the policy bound it exceeded (missing: 'refusal base_behind_main_exceeds_policy')` |
| an asserted head the repository contradicts refuses | the head asserted by the inputs is believed instead of checked | `test_an_asserted_head_the_repository_contradicts_refuses` | `not ok - a head asserted in prose that the repository contradicts refuses: expected exit 1, got 0` |
| a tampered envelope body refuses | the stored body is no longer re-digested on read | `test_a_tampered_envelope_body_refuses` | `not ok - the refusal must name the broken content address (missing: 'refusal envelope_digest_mismatch')` |
...
| duplicate attempts with no ordering refuse | unorderable repeated attempts are resolved anyway | `test_duplicate_attempts_with_no_ordering_refuse` | `not ok - the refusal must name the check whose current attempt is undecidable (missing: 'refusal ci_duplicate_attempt_undecidable')` |
...
| alias fallback resolves a later declared candidate | every candidate is still probed, but only the first is allowed to resolve | `test_alias_fallback_resolves_a_later_declared_candidate` | `not ok - a capability whose first alias is absent and whose second is present is observed, not could-not-observe: expected exit 0, got 2` |
...
| a silently dropped obligation refuses | the predecessor's active obligations are read as an empty set | `test_array_classification_registry_is_total` | `not ok - the populated registry fixture compiles to a classified envelope: expected exit 0, got 1` |
...
| a ruling that does not apply cannot authorize a resolution | ruling head applicability is no longer compared at the classify site | `test_a_ruling_that_does_not_apply_cannot_authorize_a_resolution` | `not ok - a ruling issued against another head cannot authorize anything here: expected exit 1, got 0` |
...
| a contribution that changes nothing refuses | an empty changed-file set stops refusing | `test_a_contribution_that_changes_nothing_refuses` | `not ok - the refusal must say the contribution changes nothing (missing: 'refusal changed_file_set_empty')` |
| a base the candidate does not descend from refuses | a base outside the candidate's ancestry stops refusing | `test_a_base_the_candidate_does_not_descend_from_refuses` | `not ok - the refusal must name the base the candidate does not descend from (missing: 'refusal base_not_ancestor_of_candidate')` |
...
| the generated contract page matches the catalog | the generated contract page's section heading changes | `test_the_generated_contract_page_matches_the_catalog` | `not ok - docs/contracts/review-envelope.md is stale; regenerate it with bin/fm-review-envelope.sh docs` |
...
| duplicate disposition refused, order B (earlier duplicate wins) | duplicate dispositions resolve by first-one-wins | `test_duplicate_dispositions_refuse_in_both_orders` | `not ok - the duplicate refusal must not depend on disposition order (missing: 'refusal obligation_disposition_duplicate')` |
...
| explicit null claim is distinguishable from a missing field | an explicit null claim is collapsed into absent | `test_a_structurally_malformed_envelope_is_could_not_observe` | `not ok - the readable verify record must name the malformed envelope (missing: 'unobserved envelope_unreadable')` |
...
| a structurally malformed envelope is could-not-observe | the malformed-body handler catches a narrower exception class | `test_a_structurally_malformed_envelope_is_could_not_observe` | `not ok - the readable verify record must name the malformed envelope (missing: 'unobserved envelope_unreadable')` |
| verification applicability must be declared explicitly | the applicability declaration becomes optional again | `test_verification_applicability_must_be_declared_explicitly` | `not ok - the absent applicability declaration must be named (missing: 'unobserved verification_applicability_undeclared')` |
...
A review round against this increment returned nine findings. Seven were fixed, one was fixed where a small guard sufficed and documented for the rest, and one is a stated boundary; the fixes edited two of the three measured subjects - `bin/fm-review-envelope-lib.sh` and `tests/fm-review-envelope.test.sh`, with `bin/fm-review-envelope.sh` untouched and still hashing to its recorded digest - so this campaign is invalidated by design, the campaign-artifact control refuses with a subject-digest mismatch, and the worker re-measures at the resulting head. That refusal is the control working, and the count stated under "What was measured" describes the run at the artifact's own subjects rather than these bytes until then.

- In-flight CI conclusions - IN_PROGRESS, QUEUED, PENDING and their kin, arriving non-empty through the `state` fallback - fell to NO_VERDICT and surfaced as `ci_required_check_no_verdict`, whose own text says "completed without a verdict". A check still running was reported as one that finished without deciding: the component's recurring class inverted, in-flight rendered as decided. The in-flight vocabulary is now recognized explicitly and routed to `ci_required_check_pending`; an unrecognized conclusion stays NO_VERDICT, because unknown is not the same as in-flight, and the control pins both directions so neither side can drift back.
- prepare read an explicitly null declared request identity as a mismatching claim, while the catalog and validate read the same null as none-was-claimed - and prepare writes the envelope before refusing, so the two commands gave different answers about one document. prepare now reads null as the catalog states, and a pairing control spans both readers, because two components disagreeing about one input is a defect no single-component control can catch.
- The write-once guard was a look-then-write: `os.path.exists` at entry, a non-exclusive open later, so two racing prepares could interleave and a crash mid-dump left a partial envelope.json that permanently blocked every later prepare - a guard that wedges rather than refuses. Write-once is now the filesystem's: the document is staged beside its final path and hard-linked into place, so no partial is ever visible at the final path and a losing racer refuses `envelope_exists`. Both properties are pinned by a control.
- git subprocess output was decoded through the locale's preferred encoding, so a changed path with non-ASCII bytes could decode differently, or raise, per locale - different body digests for identical facts. The decode is now explicit UTF-8, and the LOCALE AXIS of the stability claim is stated and controlled: identical candidate facts, differing locale, identical digest.
- `json.load` admitted NaN and Infinity and emission re-emitted them, so a declared record carrying NaN produced an envelope that is not strict JSON under a "canonical" digest no conforming canonicalizer can reproduce - a false identity claim. The constants are refused on load as `inputs_malformed`, and every canonical serialization now sets `allow_nan=False`.
- The diff observation requested `--abbrev=40`, which yields full object ids only under the SHA-1 OBJECT FORMAT; under sha256, ids abbreviate to a length that can grow with object-store ambiguity, so `diff_digest` could change for identical facts as unrelated objects accumulate. It now requests full unabbreviated ids, and the object-format axis of the stability claim is stated here rather than assumed.

### A control commissioned to close a vacuity shipped vacuous

The exit-code half of `test_a_malformed_but_parseable_inputs_document_is_could_not_observe` was commissioned two rounds ago, at the operator's instruction, to close a vacuity finding - and was itself vacuous on that half. The shell entrypoint injects its own `--summary-out` ahead of the caller's arguments; the case passed a second one, argparse's last-wins read redirected the classifier's summary away from the entrypoint's file, the empty summary fell through to NO_VERIFIER_RAN, and exit 2 was produced for ANY classification. `expect_code 2` measured the flag collision, not the guard - an assertion that passes for every outcome is not weak evidence, it is no evidence.

The classifier now writes every requested summary rather than only the last one named, and the control additionally pins the entrypoint's record line to the classifier's own reason, `verification_incomplete`, which the empty-summary fallback can never produce - so a wrong classification now changes both the exit code and the line. The episode is recorded beside the others rather than smoothed over, because a control commissioned to close a vacuity that ships vacuous is the most instructive instance of the class.

### Untyped but bounded: what still escapes the three-valued classification

Two escape paths named by the review were closed with a small guard at each site: validate's digest recomputations refuse `envelope_unreadable` when a hand-crafted body or outer fact cannot be canonicalized - a lone surrogate, a non-finite number - and prepare's envelope write refuses `usage_error` when the output cannot be written, as when `--out` names an existing file or an unwritable directory. No exception framework was built for the remainder, deliberately.

The honest description of that remainder is UNTYPED BUT BOUNDED: a stray path that still escapes as a traceback writes no summary, and the bash entrypoint fail-closes an absent summary to exit 2, so the contract's three-valued EXIT behaviour holds even where the classification path does not produce a typed answer.

### A deliberate boundary: a free-text authority is never cross-checked

A RESOLVED obligation disposition whose cited authority names a ruling id absent from the envelope passes on authority-and-reason presence alone: the ruling cross-check applies only to ids the envelope actually holds, and ambiguous ids refuse envelope-wide as `ruling_id_ambiguous`. This is a boundary, not an omission. The contract mandates refusal for unestablished or mismatched applicability of cited rulings the envelope carries; an authority given as free text can never be cross-checked against anything, so presence of an explicit authority and reason is the whole claim made there - and it is stated here so no reader credits the check with more than it establishes.
