...
run_spawn() {
  ...
  env -u FM_TRACE_CONTEXT \
    ...
    "$SPAWN" "$@" --mode no-mistakes --yolo off --reason-code NL_RULE_CLASSIFICATION 2>&1
}
...
run_two_level() {
  ...
  env FM_TRACE_CONTEXT="$TL_ENV_TC" TRACEPARENT="$TL_CARRIER" \
    ...
    "$SPAWN" "$worker_id" "$wproj" --mode no-mistakes --yolo off --reason-code NL_RULE_CLASSIFICATION >/dev/null 2>&1 || true
