...
# The same forge-addressed fork topology for the tests that run a real spawn.
# fm-spawn freshens a pooled worktree against origin before launch, so origin's
# FETCH url has to stay a reachable local path; only a url the spawn never
# fetches through can carry the forge identity. That is the conventional fork
# layout: origin fetches the fork locally and PUSHES the fork's forge url, while
# a separate `upstream` remote carries the upstream forge url. Both venues are
# still read from real forge urls, and both directions are already proven on the
# fetch/push-split layout by the derivation tests above.
make_spawnable_fork_repo() {  # <name> <fleet-commits>
  local name=$1 fleet=$2 repo
  repo=$(make_upstream_remote_repo "$name" "$fleet")
  git -C "$repo" remote set-url origin "$TMP_ROOT/$name/fork.git"
  git -C "$repo" remote set-url --push origin "https://github.com/fixture-fork/$name.git"
  printf '%s\n' "$repo"
}
...
test_spawn_records_the_venue_its_contribution_target_names() {
  local repo wt home fakebin id out status slot contrib
  repo=$(make_spawnable_fork_repo venue-spawn 3)
  ...
  touch "$home/state/.last-watcher-beat"
  slot=$(git -C "$repo" rev-parse main)
  contrib=$(git -C "$repo" rev-parse upstream/main)
...
test_spawn_records_the_fork_venue_for_a_retargeted_task() {
  ...
  repo=$(make_spawnable_fork_repo venue-retarget 3)
