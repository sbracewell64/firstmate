...
test_spawn_secondmate_harness_model_token() {
  ...
  launch=$(cat "$launchlog")
  assert_contains "$launch" "claude --permission-mode dontAsk --settings '{\"permissions\":{\"allow\":[\"Bash\",\"Read\",\"Edit\",\"Write\",\"MultiEdit\",\"Glob\",\"Grep\",\"WebFetch\",\"WebSearch\"],\"defaultMode\":\"dontAsk\"}}' --model 'opus'" \
...
test_spawn_secondmate_harness_model_and_effort_tokens() {
  ...
  launch=$(cat "$launchlog")
  assert_contains "$launch" "claude --permission-mode dontAsk --settings '{\"permissions\":{\"allow\":[\"Bash\",\"Read\",\"Edit\",\"Write\",\"MultiEdit\",\"Glob\",\"Grep\",\"WebFetch\",\"WebSearch\"],\"defaultMode\":\"dontAsk\"}}' --model 'opus' --effort 'high'" \
...
test_spawn_fallback_chain_and_crew_scout_unaffected() {
  ...
  PATH="$fakebin:$BASE_PATH" TMUX="fake,1,0" CLAUDECODE=1 \
    ...
    "$ROOT/bin/fm-spawn.sh" "$id" "$proj" --mode no-mistakes --yolo off --reason-code NL_RULE_CLASSIFICATION >/dev/null 2>&1
