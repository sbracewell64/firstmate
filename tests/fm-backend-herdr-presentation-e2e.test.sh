...
spawn_task() {  # <id> <home> <project>
  ...
  FM_GATE_REFUSE_BYPASS=1 FM_SPAWN_NO_GUARD=1 FM_HOME="$home" FM_ROOT_OVERRIDE="$ROOT" \
    "$ROOT/bin/fm-spawn.sh" "$id" "$project" "sh -c 'echo fm-status-event.v1 verb=working phase=setup summary=stub worker started >> $home/state/$id.status; sleep 120'" --mode no-mistakes --yolo off --reason-code NL_RULE_CLASSIFICATION --backend herdr
}
