...
OLD_BIN_UNCHANGED_SIBLINGS="fm-gate-refuse-lib.sh fm-guard.sh fm-lock-lib.sh fm-landed-lib.sh fm-tasks-axi-lib.sh fm-pr-lib.sh fm-tangle-lib.sh fm-tmux-lib.sh fm-composer-lib.sh fm-launch-lib.sh fm-wake-lib.sh fm-classify-lib.sh fm-autonomy-lib.sh fm-status-event-lib.sh fm-supervision-lib.sh fm-ff-lib.sh fm-config-inherit-lib.sh fm-project-mode.sh fm-harness.sh fm-crew-state.sh fm-nm-run-lib.sh fm-timeout-lib.sh fm-decision-hold.sh fm-backend.sh fm-operational-input.sh fm-public-followup-lib.sh fm-secondmate-registry-lib.sh fm-x-lib.sh fm-admission-lib.sh fm-task-axis-lib.sh"
...
make_spawn_fakebin() {  # <dir> <fake-worktree-path> -> echoes fakebin dir
  ...
  mkdir -p "$fb"
  # Built by hand rather than through fm_fakebin, so install the stub worker.
  fm_fake_deliver_install "$fb"
  cat > "$fb/tmux" <<SH
#!/usr/bin/env bash
set -u
{ printf 'tmux'; for a in "\$@"; do printf '\\x1f%s' "\$a"; done; printf '\\n'; } >> "\${FM_TMUX_LOG:?}"
...
run_spawn_symlink_case() {  # <label> <physical|logical>
  ...
  out=$(run_spawn_case "$ROOT" "$fb" "$log" "$state" "$data" "$config" "$proj" -- "$id" "$proj" claude --mode no-mistakes --yolo off --reason-code NL_RULE_CLASSIFICATION 2>&1)
  rc=$?
...
test_spawn_refuses_unknown_backend_flag() {
  ...
  out=$(FM_ROOT_OVERRIDE='' FM_HOME='' FM_STATE_OVERRIDE='' FM_DATA_OVERRIDE='' \
    ...
    "$ROOT/bin/fm-spawn.sh" nope-backend-z1 projects/none claude --mode no-mistakes --yolo off --reason-code NL_RULE_CLASSIFICATION --backend bogus 2>&1)
  status=$?
...
test_spawn_refuses_codex_app_backend_flag() {
  ...
  out=$(FM_ROOT_OVERRIDE='' FM_HOME='' FM_STATE_OVERRIDE='' FM_DATA_OVERRIDE='' \
    FM_PROJECTS_OVERRIDE='' FM_CONFIG_OVERRIDE='' FM_SPAWN_NO_GUARD=1 \
    "$ROOT/bin/fm-spawn.sh" nope-codex-app-z1 projects/none claude --mode no-mistakes --yolo off --reason-code NL_RULE_CLASSIFICATION --backend codex-app 2>&1)
  status=$?
...
test_spawn_refuses_unknown_fm_backend_env() {
  ...
  out=$(FM_ROOT_OVERRIDE='' FM_HOME='' FM_STATE_OVERRIDE='' FM_DATA_OVERRIDE='' \
    ...
    "$ROOT/bin/fm-spawn.sh" nope-backend-z2 projects/none claude --mode no-mistakes --yolo off --reason-code NL_RULE_CLASSIFICATION 2>&1)
  status=$?
...
test_spawn_default_backend_writes_no_meta_field() {
  ...
  mkdir -p "$data/$id"; printf 'Task brief: exercise the spawn path end to end.\n' > "$data/$id/brief.md"
  ...
  out=$(PATH="$fb:$PATH" FM_ROOT_OVERRIDE="$ROOT" \
    ...
    "$ROOT/bin/fm-spawn.sh" "$id" "$proj" claude --mode no-mistakes --yolo off --reason-code NL_RULE_CLASSIFICATION --backend tmux 2>&1)
...
test_spawn_autodetect_nesting_resolves_tmux_silently() {
  ...
  out=$(PATH="$fb:$PATH" FM_ROOT_OVERRIDE="$ROOT" \
    ...
    "$ROOT/bin/fm-spawn.sh" "$id" "$proj" claude --mode no-mistakes --yolo off --reason-code NL_RULE_CLASSIFICATION 2>&1)
