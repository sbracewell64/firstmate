...
install_guard_scripts() {
  ...
  cp "$ROOT/bin/fm-status-event-lib.sh" "$dir/bin/fm-status-event-lib.sh"
  mkdir -p "$dir/docs"
...
test_grok_adapter_invalid_inputs_start_neither_path() {
  ...
  out=$(printf '%s' '{"sessionId":"x","stopHookActive":false}' | PATH="$fakebin:$PATH" FM_HOME="$dir" GROK_WORKSPACE_ROOT="$TMP_ROOT/missing-grok-root" bash "$dir/bin/fm-turnend-guard-grok.sh" 2>&1); status=$?
