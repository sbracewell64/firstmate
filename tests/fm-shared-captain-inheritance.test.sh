...
EOF
  ...
  fakebin=$(make_fake_spawn_toolchain "$w")
  fm_fake_exit0 "$fakebin" node gh-axi chrome-devtools-axi lavish-axi gh no-mistakes tasks-axi quota-axi
...
EOF
  fakebin=$(make_fake_spawn_toolchain "$w")
  fm_fake_exit0 "$fakebin" node gh-axi chrome-devtools-axi lavish-axi gh no-mistakes tasks-axi quota-axi pgrep
  fm_fake_treehouse "$fakebin"
