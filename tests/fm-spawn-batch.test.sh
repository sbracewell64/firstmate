  ...
  while IFS='|' read -r label use_override id; do
    ...
    if [ "$use_override" = yes ]; then
      out=$(FM_ROOT_OVERRIDE='' FM_STATE_OVERRIDE='' FM_DATA_OVERRIDE='' FM_CONFIG_OVERRIDE='' \
        ...
        "$SPAWN" "$id" projects/alpha codex --mode no-mistakes --yolo off --reason-code NL_RULE_CLASSIFICATION 2>&1)
    else
    ...
    expected="FM_SPAWN_BRIEF_MISSING: no brief at $home/data/$id/brief.md"
