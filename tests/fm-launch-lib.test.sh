...
PRIMARY_HARNESSES=(claude codex opencode pi grok)
...
test_ship_and_scout_templates_are_pinned() {
  ...
  for kind in ship scout; do
    ...
    assert_template "$kind" pi 'pi __MODELFLAG____EFFORTFLAG__-e __PIEXT__ "$(__OPINPUT__ encode launch-brief < __BRIEF__)"'
  ...
  pass "launch_template: ship and scout templates are unchanged for all six verified adapters"
}
...
test_secondmate_templates_are_pinned() {
  # Only codex and pi differ from the ship shape: codex drops the per-task
  # notify hook and pi points at the secondmate home's own primary extensions.
  ...
  assert_template secondmate pi 'pi __MODELFLAG____EFFORTFLAG__-e __PITURNEND__ -e __PIWATCH__ "$(__OPINPUT__ encode launch-brief < __BRIEF__)"'
  local h
  ...
  pass "launch_template: secondmate templates are unchanged (codex and pi differ, the rest match ship)"
}
...
test_primary_pi_template_is_pinned() {
  ...
  assert_template primary pi 'pi __MODELFLAG____EFFORTFLAG__'
  pass "launch_template: the pi primary template is pinned"
}
...
test_primary_kimi_refuses() {
  # README.md:61 lists only Claude Code, Grok, Pi, Codex, and OpenCode as verified
  # primary harnesses (docs/configuration.md:177 defers that narrower set to
  # README), and __KIMIBIN__ is resolvable by bin/fm-spawn.sh alone, which never
