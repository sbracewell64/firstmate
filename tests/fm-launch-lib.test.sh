...
test_effort_flag_per_harness_vocabulary() {
  ...
  [ "$(effort_flag_for_harness codex max)" = "-c 'model_reasoning_effort=\"max\"' " ] || fail "codex must carry max, not drop it silently"
  [ -z "$(effort_flag_for_harness codex ultra)" ] || fail "codex must omit ultra, which no evidence supports"
...
run_sandboxed_spawn() {
  ...
  FM_HOME="$sandbox/home" FM_SPAWN_NO_GUARD=1 FM_BACKEND=tmux \
    ...
    --mode no-mistakes --yolo off --harness "$harness" --reason-code NL_RULE_CLASSIFICATION 2>&1
}
