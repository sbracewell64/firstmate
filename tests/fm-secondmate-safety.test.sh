...
test_fm_home_parameterization() {
  ...
  # fm-pr-check.sh reads the landing repository from the project's origin push
  # URL, so the recorded project is a real clone of the pull request's repository.
  mkdir -p "$home_one/project"
  git -C "$home_one/project" init -q || fail "could not initialize the home project fixture"
  git -C "$home_one/project" remote add origin https://github.com/example/repo.git \
    || fail "could not set the home project origin"
