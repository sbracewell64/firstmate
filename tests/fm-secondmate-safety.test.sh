...
test_remote_reclaim_refuses_a_malformed_residue_path() {
  local case_dir case_home out
  case_dir="$TMP_ROOT/reclaim-shape"
  case_home="$case_dir/home"
  mkdir -p "$case_home/state/parent-route"

  out=$(reclaim_guard "$case_home" "$case_home" "")
  [ "$(reclaim_rc "$case_home" "$case_home" "")" -ne 0 ] || fail "an empty residue path must be refused"
  assert_contains "$out" "path is empty" "the refusal must name the empty path"

  out=$(reclaim_guard "$case_home" "$case_home" "state/parent-route")
  [ "$(reclaim_rc "$case_home" "$case_home" "state/parent-route")" -ne 0 ] || fail "a relative residue path must be refused"
  assert_contains "$out" "not an absolute path" "the refusal must name the relative path"

  out=$(reclaim_guard "$case_home" "$case_home" "$case_home/state/other")
  [ "$(reclaim_rc "$case_home" "$case_home" "$case_home/state/other")" -ne 0 ] || fail "a path not ending in state/parent-route must be refused"
  assert_contains "$out" "does not end in state/parent-route" "the refusal must name the required suffix"

  assert_present "$case_home/state/parent-route" "no refusal may remove anything"
...
test_remote_reclaim_refuses_a_residue_outside_the_recorded_home() {
  local case_dir case_home other out
  case_dir="$TMP_ROOT/reclaim-prefix"
  case_home="$case_dir/home"
  other="$case_dir/other"
  mkdir -p "$case_home/state/parent-route" "$other/state/parent-route"

  out=$(reclaim_guard "$case_home" "$other" "$case_home/state/parent-route")
  [ "$(reclaim_rc "$case_home" "$other" "$case_home/state/parent-route")" -ne 0 ] \
  ...
  out=$(reclaim_guard "$case_home" "" "$case_home/state/parent-route")
  [ "$(reclaim_rc "$case_home" "" "$case_home/state/parent-route")" -ne 0 ] \
...
test_remote_reclaim_refuses_a_residue_that_resolves_away() {
  local case_dir case_home outside out
  case_dir="$TMP_ROOT/reclaim-resolve"
  case_home="$case_dir/home"
  outside="$case_dir/outside"
  mkdir -p "$case_home/state" "$outside/keepme"

  ln -s "$outside" "$case_home/state/parent-route"
  out=$(reclaim_guard "$case_home" "$case_home" "$case_home/state/parent-route")
  [ "$(reclaim_rc "$case_home" "$case_home" "$case_home/state/parent-route")" -ne 0 ] \
  ...
  rm -f "$case_home/state/parent-route"

  ln -s "$HOME" "$case_home/state/parent-route"
  out=$(reclaim_guard "$case_home" "$case_home" "$case_home/state/parent-route")
  [ "$(reclaim_rc "$case_home" "$case_home" "$case_home/state/parent-route")" -ne 0 ] \
...
test_remote_reclaim_refuses_an_unresolvable_recorded_home() {
  local case_dir case_home out rc
  ...
  out=$(
    (
      FM_HOME="$case_home" . "$ROOT/bin/fm-remote-secondmate-control.sh" >/dev/null 2>&1
      set +e
      cd() { [ "${1-}" != "$case_home" ] || return 1; builtin cd "$@"; }
      retire_control_state_residue "$case_home" "$case_home/state/parent-route" ios 2>&1
      printf 'rc=%s\n' "$?"
    )
  )
  rc=${out##*rc=}
  ...
  assert_present "$case_home/state/parent-route" "a refused reclaim removes nothing"
...
test_remote_reclaim_refuses_until_the_home_deletion_succeeded() {
  ...
  rm -f "$case_home/.fm-secondmate-home"
  [ "$(reclaim_rc "$case_home" "$case_home" "$case_home/state/parent-route")" -eq 0 ] \
    || fail "reclaim must succeed once the home deletion has happened"
  assert_absent "$case_home/state/parent-route" "the residue must be gone"
  assert_absent "$case_home" "and the home it was the last thing in"
