...
env -u TMUX -u FM_BACKEND PATH="$PATH" HERDR_ENV=1 \
  ...
  FM_SPAWN_NO_GUARD=1 \
  "$ROOT/bin/fm-spawn.sh" "$ID" "$PROJ" "sh -c 'echo autodetect-smoke-ok; echo fm-status-event.v1 verb=working phase=setup summary=stub worker started >> $STATE/$ID.status'" --mode no-mistakes --yolo off --reason-code NL_RULE_CLASSIFICATION \
  >"$OUT_FILE" 2>"$ERR_FILE"
status=$?
