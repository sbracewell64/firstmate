...
# and neither can say the case never got to look - a required loader the host
# never supplied, a fixture that could not establish the overlap it was about to
# measure, a bounded wait that spent its envelope on a saturated box. Reporting
...
# a kill or a cleanup would otherwise destroy: the probe that refused, the
# version it refused under, elapsed wall time, the configured budget. Capture it
# BEFORE tearing the subject down.
#
# It does not end the case, because only the case knows what it may still
# assert. The caller ends there itself, usually with a bare `return`.
#
# fail() is unchanged and still owns a genuine assertion failure. A property the
