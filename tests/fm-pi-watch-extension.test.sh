...
test_spawn_template_mentions_pi_watch_placeholder() {
  ...
  # The template itself lives in bin/fm-launch-lib.sh, the single owner of every
  # launch command; fm-spawn.sh only substitutes its placeholders.
  template=$(cat "$ROOT/bin/fm-launch-lib.sh")
  assert_contains "$template" "-e __PITURNEND__ -e __PIWATCH__" "Pi secondmate launch template does not include both primary extensions"
